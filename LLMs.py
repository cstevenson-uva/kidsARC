...
import together
together.api_key = "655e6b8b8d4391221ffbba217d3a188729f9438a8cf971995661e1c2acad789d"
...
if __name__ == '__main__':
    ...
    system_prompt_vision = "You are a helpful assistant that solves analogy making puzzles. Only give the answer, no other words or text. Provide your response using the following color pallette:\n\
    0: black\n\
    1: firebrick\n\
    2: darkorange\n\
    3: gold\n\
    ...
    models = ['gpt-4-0613', 'gpt-3.5-turbo-0301', 'gpt-4-vision-preview']
    ...
    for model in models[:-1]:
        ...
        for task in range(1,17):  
            response = gpt_call(
                input=task_to_input(task, input_str=''), 
                model=model, 
        ...
        json.dump(results_dict, open('data/model/GPT_imagesPrompt.json', 'w'))

    # gpt vision
    for task in range(1,17):  
        response = gpt_call_vision(
            input_text = starting_str, 
            input_img = task_to_base64(task),
            system_prompt_text = system_prompt_vision
        )
        
        results_dict['gpt-4-vision-preview'][str(task)]['response'].append(response)
        results_dict['gpt-4-vision-preview'][str(task)]['correct'].append(check_response(task, response))
        print(f'Task {task} - Correct: {bool(results_dict['gpt-4-vision-preview'][str(task)]['correct'][0])}')
