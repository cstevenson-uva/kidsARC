...
for i in range(len(df)):
    ...
    if len(response_to_array(df['response'][i])) != len(task_to_array(df['itemid'][i])):
        ...
        print(df['model'][i], df['itemid'][i])
        count += 1

'''
togethercomputer/Llama-2-7B-32K-Instruct 9
togethercomputer/RedPajama-INCITE-Chat-3B-v1 14
togethercomputer/falcon-40b-instruct 10
togethercomputer/falcon-40b-instruct 13
'''

### GPT and GPT_vision
gpt_data = json.load(open('data/model/GPT.json', 'r'))
gpt_vision_data = json.load(open('data/model/GPT_vision.json', 'r'))
gpt_vision_data = {'gpt-4-vision_'+k[21:]: v for k, v in gpt_vision_data.items()}
gpt_data.update(gpt_vision_data)


for model in list(gpt_data.keys()): 
    for task in gpt_data[model]:
        gpt_data[model][task]['response'] = filter_response(gpt_data[model][task]['response'][0])
        if type(gpt_data[model][task]['response']) == float: # If the response is nan
            continue
        gpt_data[model][task]['response'] = gpt_data[model][task]['response'].replace('[', '').replace(']', '').replace(' ', '')

gpt_df = pd.DataFrame([
    {**{'model': model, 'itemid': int(task)}, **gpt_data[model][task]}
    for model in gpt_data
    for task in gpt_data[model]
])

for i in range(len(gpt_df)):
    if type(gpt_df['response'][i]) == float:
        gpt_df.loc[i, 'correct'] = 0
    else:
        gpt_df.loc[i, 'correct'] = int(np.array_equal(np.array([int(x) for x in gpt_df['response'][i]]), task_to_array(gpt_df['itemid'][i])))
...
for i in range(len(gpt_df)):
    if type(gpt_df['response'][i]) == float:
        continue
    if len(response_to_array(gpt_df['response'][i])) != len(task_to_array(gpt_df['itemid'][i])):
        gpt_df.loc[i, 'response'] = 'NA'
        print(gpt_df['model'][i], df['itemid'][i])
        count += 1

df = pd.concat([df, gpt_df], ignore_index=True)
